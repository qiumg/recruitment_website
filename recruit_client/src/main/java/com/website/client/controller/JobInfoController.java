...
public class JobInfoController {
    ...
    @ResponseBody
//    HttpServletRequest request, 加上
    public Map<String,Object> addJobInfo( @RequestParam("positionType") String positionType,
...
//        Integer c_id = (Integer) request.getSession().getAttribute("cid");  加上
        Date date=new Date();
        Map map = new HashMap();
    ...
    public String selectHisJobByCid(HttpServletRequest request,ModelMap map){
        //        Integer c_id = (Integer) request.getSession().getAttribute("cid");
        Map map1=new HashMap();
        map1.put("c_id",1); //改

        Map m =restTemplate.getForObject("http://PROVIDER-SERVER/jobInfo/selectHisJob?c_id={c_id}",HashMap.class,map1);
